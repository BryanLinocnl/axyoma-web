...
export type UsageChartPoint = { date: string; credits: number }
...
function UsageTooltip({ active, payload, creditBrl }: { active?: boolean; payload?: { payload: UsageChartPoint }[]; creditBrl: number }): React.JSX.Element | null {
  ...
  return (
    <div className="border-border/50 bg-background grid min-w-36 gap-1 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl">
      ...
      <span className="text-muted-foreground">≈ R$ {fmt(creditsToBRL(point.credits, creditBrl))}</span>
    </div>
  )
}

export function UsageChart({ data, creditBrl = FALLBACK_CREDIT_BRL }: { data: UsageChartPoint[]; creditBrl?: number }): React.JSX.Element {
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        <ChartContainer config={chartConfig} className="aspect-auto h-[250px] w-full">
          <AreaChart data={filtered}>
            ...
            />
            <ChartTooltip cursor={false} content={<UsageTooltip creditBrl={creditBrl} />} />
            <Area dataKey="credits" type="natural" fill="url(#fillCredits)" stroke="var(--color-credits)" />
          </AreaChart>
        </ChartContainer>
      </CardContent>
    </Card>
