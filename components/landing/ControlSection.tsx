    ...
    <div className="pt-2">
      ...
      >
        <div className="flex items-baseline justify-between">
          <span className="text-[12px] font-medium">{t('artPlanTitle')}</span>
...
function RunTrail(): React.JSX.Element {
  const t = useTranslations('control')
  // Só o VERBO é traduzido. O objeto (`frete.ts`, `npm test`, `PR #128`) é
  // nome de arquivo e comando — traduzir isso descreveria um projeto que não
  // existe.
  const steps: [string, string][] = [
    [t('artRead'), '3 arquivos'],
    [t('artEdited'), 'frete.ts'],
    [t('artRan'), 'npm test'],
    [t('artOpened'), 'PR #128'],
  ]
  return (
  ...
  return (
    <div className="pt-4">
      <div
        ...
        style={{ border: '1px solid var(--hairline)' }}
      >
        <p className="text-[12.5px] font-medium">{t('artPermissionTitle')}</p>
...
const CARDS: { id: 'plan' | 'trail' | 'permission'; art: () => React.JSX.Element }[] = [
  { id: 'plan', art: PlanProposal },
  { id: 'trail', art: RunTrail },
  { id: 'permission', art: PermissionPrompt },
]
