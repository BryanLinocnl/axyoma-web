...
// (`billingType: 'PIX'`), então cartão não é opção hoje — habilitar no painel do
// Asaas não basta. Devolver a menção em `ledger.balanceBody` quando a função
// mudar (nos DOIS catálogos).
const POINTS = ['trail', 'cost', 'balance'] as const
        ...
        <div className="mt-14 grid gap-12 lg:grid-cols-[0.92fr_1.08fr] lg:gap-16">
          ...
          <dl className="flex flex-col">
            {POINTS.map((point, i) => (
              ...
              >
                <dt className="text-[16.5px] font-semibold tracking-[-0.015em]">
                  {t(`${point}Title`)}
                </dt>
                <dd
